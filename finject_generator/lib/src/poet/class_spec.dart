...
class ClassSpec implements Spec {
  ...
  ClassSpec.build(
    ...
    if (constructorBuilder != null) {
      constructors.addAll(constructorBuilder(this));
    }
    metas ??= [];
    properties ??= [];
    getters ??= [];
  ...
  String code({Map<String, Object> args = const {}}) {
    ...
    if (hasGeneric) {
      inner.write("<${generics.map((o) => o.fullTypeName).join(", ")}>");
    }
    if (superClass != null) inner.write(' extends $superClass');
    if (implementClasses.isNotEmpty) {
      inner.write(' implements ${implementClasses.map((o) => o).join(', ')}');
    }
    if (mixinClasses.isNotEmpty) {
      inner.write(' with ${mixinClasses.join(', ')}');
    }
    inner.writeln(' {');
    var blocks = [];
  ...
  String code({Map<String, Object> args = const {}}) {
    ...
    switch (mode) {
      ...
      case ConstructorMode.namedFactory:
        raw +=
            'factory ${owner.className}.$name(${collectParameters(parameters)})';
        break;
    }
    if (inherit != null) {
      raw += ' : $inherit';
